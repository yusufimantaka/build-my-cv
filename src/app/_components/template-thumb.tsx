...
export default function TemplateThumb({ template }: { template: CVTemplate }) {
    ...
    return (
        <div className="relative mx-auto aspect-[210/297] h-44 w-auto overflow-hidden rounded-sm border border-[#171717]/10 bg-white">
            {/* Jalur sidebar kiri */}
            {sidebarLayout && (
            ...
            <div className={sidebarLayout ? "pl-1/4" : ""}>
                ...
                <div className="px-3 pb-3">
                    ...
                    {bar && <div className="mt-3 h-2 w-full rounded-sm bg-[#ddd8cd]" />}
                    <div className="mt-1.5 h-1.5 w-full rounded-sm bg-[#e4ddcd]" />
                    <div className="mt-1 h-1.5 w-2/3 rounded-sm bg-[#e4ddcd]" />
                    {!bar && <div className="mt-3 h-1.5 w-1/3 rounded-sm" style={{ background: accent, opacity: 0.5 }} />}
